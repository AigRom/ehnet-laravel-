...
@if($link)
    <a
        href="{{ $link }}"
        class="flex items-center gap-3 rounded-xl border border-zinc-200 bg-white p-2.5 transition hover:border-green-300 hover:shadow-sm sm:p-3"
    >
@else
    <div
        class="flex items-center gap-3 rounded-xl border border-zinc-200 bg-white p-2.5 opacity-90 sm:p-3"
    >
@endif

    <div class="h-14 w-14 shrink-0 overflow-hidden rounded-lg bg-zinc-100 sm:h-16 sm:w-16">
        @if($coverImage)
            <img
                src="{{ $coverImage }}"
            ...
            >
        @else
            <div class="flex h-full w-full items-center justify-center text-[10px] text-zinc-500 sm:text-[11px]">
                {{ __('Pilt puudub') }}
            </div>
        @endif
    </div>
    ...
    <div class="min-w-0 flex-1">
        <div class="flex min-w-0 items-center gap-2">
            <div class="truncate text-sm font-semibold text-zinc-900 flex-none max-w-[55%] sm:max-w-[60%]">
                {{ $title }}
            </div>

            @if($locationLabel)
                <div class="flex min-w-0 items-center gap-1 text-[11px] text-zinc-500">
                    <svg class="h-3.5 w-3.5 shrink-0 text-zinc-400" viewBox="0 0 20 20" fill="currentColor">
